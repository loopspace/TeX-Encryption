...
\def\do_clone_enigma#1{%
    \directlua{
      local enigma = packagedata.enigma
      local current_args = enigma.retrieve_raw_args(\!!bs\parent_enigma_id\!!es)
      enigma.save_raw_args(current_args, \!!bs\current_enigma_id\!!es)
      enigma.new_callback(
        enigma.new_machine(\!!bs\current_enigma_id\!!es),
        \!!bs\current_enigma_id\!!es)
...
\def\inheritenigma#1#2{%
  \defineenigma{#2}%
  \bgroup
    \edef\current_enigma_id{#2}
    \edef\parent_enigma_id{#1}
    \luatexcatcodetable \enigmasetupcatcodes
    \do_clone_enigma%
}
%D \stopdocsection
